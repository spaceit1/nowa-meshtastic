...
import { FiPlus, FiGlobe, FiEdit2, FiTrash2, FiLock, FiUnlock } from 'react-icons/fi';
...
interface UserMessageTemplateFormData {
  name_pl: string;
  name_en: string;
  name_uk: string;
  name_ru: string;
...
const UserMessageTemplatesPanel: React.FC = () => {
  const { t } = useLanguage();
  const { user } = useAuth();
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<UserMessageTemplate | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isTranslating, setIsTranslating] = useState(false);
  const [isEditTranslating, setIsEditTranslating] = useState(false);
  const [addFormData, setAddFormData] = useState<UserMessageTemplateFormData>({
    name_pl: '',
    name_en: '',
    name_uk: '',
    name_ru: '',
  ...
  const [editFormData, setEditFormData] = useState<UserMessageTemplateFormData>({
    name_pl: '',
    name_en: '',
    name_uk: '',
    name_ru: '',
  ...
  const openEditModal = (template: UserMessageTemplate) => {
  ...
  const openDeleteModal = (template: UserMessageTemplate) => {
    setSelectedTemplate(template);
  ...
  return (
    <div className="space-y-6 mx-auto">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">{t("userMessageTemplates")}</h2>
          ...
          {templates.map((template) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-auto">
                ...
                {template.is_public ? (
                  <FiUnlock className="w-4 h-4 text-green-500" title={t('publicTemplate')} />
                ) : (
                  <FiLock className="w-4 h-4 text-red-500" title={t('privateTemplate')} />
                )}
...
export default UserMessageTemplatesPanel; 